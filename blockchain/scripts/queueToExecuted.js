...
async function main() {
  ...
  const proposalEta = await govern.proposalEta(proposalId);
  const currentBlockTime = (await ethers.provider.getBlock("latest")).timestamp;
  console.log("Current Time:", currentBlockTime.toString());
  console.log("proposal Eta:", proposalEta);
  const timeToBePassed = proposalEta - BigInt(currentBlockTime);
